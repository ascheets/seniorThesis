...
In contrast, surface forces are forces that act upon the surface of the volume and thus have a greater magnitude at the surface.
Surface forces are transmitted through the volume, but diminish in magnitude moving away from the surface of the fluid.
...
Gravity, pressure, and shear are the only sources of pressure needed for simulation of fluid flow in a pipe.
Electromagnetic forces are also body forces in the context of fluids, but we won't be considering those forces.

We have established that the fluid forces we are interested in includes gravity, pressure stress, and shear stress.
We would like to equate these forces to the rate of change of momentum, which we have a set of component equations for (\ref{eq:momComps}).
Thus, we need to figure out what these relevant forces look like in the x, y, and z directions.


The differential form of pressure force:

\begin{equation}
F_p = \nabla{p(x,y,z)}
\end{equation}

The differential form of shear force:
